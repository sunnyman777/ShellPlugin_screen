...
public class MS3DLoader {
	 ...
	 */
	public boolean load(InputStream is, AbsMS3DModel model) {
		try {
			...
			MS3DJoint[] joints = loadJointList();
			model.mpJoints = setupJoints(joints, model.mpVertices, model.mpTriangles);
	...
	private final MS3DVertex[] loadVertexList() throws IOException {
		...
		for (int i = 0; i < numVerts; i++) {
			...
			final Vector3f vertex = new Vector3f(this.mLedis.readFloat(), this.mLedis.readFloat(),
	...
	private final MS3DMaterial[] loadMaterialList() throws IOException {
		...
		for (int x = 0; x < numMaterials; x++) {
			...
			mat.setAmbient(new Color4f(this.mLedis.readFloat(), this.mLedis.readFloat(), this.mLedis.readFloat(),
					this.mLedis.readFloat()));
			mat.setDiffuse(new Color4f(this.mLedis.readFloat(), this.mLedis.readFloat(), this.mLedis.readFloat(),
					this.mLedis.readFloat()));
			mat.setSpecular(new Color4f(this.mLedis.readFloat(), this.mLedis.readFloat(), this.mLedis.readFloat(),
					this.mLedis.readFloat()));
			mat.setEmissive(new Color4f(this.mLedis.readFloat(), this.mLedis.readFloat(), this.mLedis.readFloat(),
					this.mLedis.readFloat()));
	...
	private final MS3DJoint[] loadJointList() throws IOException {
		...
		for (int x = 0; x < numJoints; x++) {
			...
			for (int i = 0; i < numKeyFramesRot; i++) {
				...
				keyFrameRot.setRotation(new Vector3f(this.mLedis.readFloat(), this.mLedis.readFloat(), this.mLedis
						.readFloat()));
			...
			for (int i = 0; i < numKeyFramesPos; i++) {
				...
				keyFramePos.setPosition(new Vector3f(this.mLedis.readFloat(), this.mLedis.readFloat(), this.mLedis
						.readFloat()));
	 ...
	 * @throws IOException
	 */
	private void loadExtra(AbsMS3DModel model) throws IOException {
	...
	private final Joint[] setupJoints(final MS3DJoint[] jointList, final MS3DVertex[] vertList,
