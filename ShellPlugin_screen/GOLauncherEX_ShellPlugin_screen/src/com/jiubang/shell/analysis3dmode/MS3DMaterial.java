...
final class MS3DMaterial {
	...
	/**
	 * Creates new MS3DMaterial. The object is initialized by the provided
	 * parameters
	 * 
	 * @param name
	 ...
	 * @param transparency
	 *            the transparency level of the material
	 * @param mode
	 *            unused
	 * @param mTextureName
	 *            the filename of the primary textureName
	 * @param alpha
	 *            the filename of the secondary textureName
	 */
	MS3DMaterial(final String name, final Color4f ambient,
	        final Color4f diffuse, final Color4f specular,
	        final Color4f emissive, final float shininess,
	        final float transparency, final byte mode, final String texture,
	        final String alpha) {
		setName(name);
		setAmbient(ambient);
		setDiffuse(diffuse);
	...
	/**
	 * A utility function to convert Milkshape filenames to a
	 * platform-independent format. Milkshape stores all filenames in a DOS
	 * format.
	 * 
	 * @param filename
