 ...
 *
 */
public class GLSenseWorkspace extends GLViewGroup
		implements
			GLCardLayout.ICardEventListener,
	...
	private GLCardLayout mHomeCard;
	private GLCardLayout mOpratorCardLayout;
	...
	private int mStatus;
	private final int mMovingDuration = 250;
	private final int mReplaceAnimationDuration = 300;
	private int mRemoveCardId = -1; // 被删除的卡片ID
	private int mLeaveCardId = -1; // 离开显示的卡片ID

	// 作离开动画的参数
	private GLView mLeavingView;
	private float mCx;
	private float mCy;
	private float mScale;
	public static final int LEAVE_DURATION = 180;
	...
	private static final int HANDLE_REPLACEFINISH = 1000;  //完成替换,更改id
	...
	public void setCaptionY() {
		...
		if (sCardCount <= 3) {
			...
			mListener.setCaptionText(mCaptionText, 1);
		} else if (sCardCount <= 6) {
			mDragCaptionTop += (sSpaceY + sCardHeight) * 2 + sSpaceY;
			mListener.setCaptionText(mCaptionText, 2);
		} else {
			mDragCaptionTop = NO_SHOW_CAPTION;
			mListener.setCaptionText(mCaptionText, 0);
		}
	 ...
	 * 绘制Home键提示信息
	 */
	private void drawHomeCaption(final GLCanvas canvas) {
	 ...
	 * 绘制拖拽提示文字
	 */
	private void drawCaption(final GLCanvas canvas) {
	 ...
	 *            属性
	 */
	public GLSenseWorkspace(Context context, AttributeSet attrs) {
	 ...
	 *            样式
	 */
	public GLSenseWorkspace(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.GLSenseWorkspace,
				defStyle, 0);
		mBackgroudColor = a.getColor(R.styleable.GLSenseWorkspace_gl_backgroundColor, 0xa6000000);
		mRows = a.getInt(R.styleable.GLSenseWorkspace_gl_cardRows, 3);
		mColumns = a.getInt(R.styleable.GLSenseWorkspace_gl_cardCols, 3);
		sSpaceScaleX = a.getInt(R.styleable.GLSenseWorkspace_gl_cardSpaceScaleX, 160);
		sSpaceScaleY = a.getInt(R.styleable.GLSenseWorkspace_gl_cardSpaceScaleY, 160);
		mRowsMulColumns = mRows * mColumns;
		sMarginTop = a.getDimensionPixelSize(R.styleable.GLSenseWorkspace_gl_marginTop, 25);
		a.recycle();
	 ...
	 * 添加组件到指定屏幕
	 */
	public void addInScreen(GLCardLayout card) {
	...
	public static float sPreViewScaleFactor; // CellLayout缩放比例
	public static boolean sNeedScaleWidget = false; // 是否需要缩放widget
	static float sX;
	static float sY;
	...
	void getDesiredLayoutConfigure(int width, int height, int iterator) {
		...
		if (GLSense.sScreenFrameStatus) {
			sMarginTop = sMarginTop - StatusBarHandler.getStatusbarHeight();
		} else {
			if (!GLSense.isEnterFromDragView()) {
				if (!GLSenseWorkspace.showStatusBar) {
					sMarginTop = sMarginTop - StatusBarHandler.getStatusbarHeight();
//					if (!landscapeMode) {
//						sMarginTop = sMarginTop - StatusBarHandler.getStatusbarHeight();
//					} else {
...
//					}
		...
		sCardHeight = (sHeight - sMarginTop - topD - sMarginBottom - sSpaceY * 2) / mRows ;
		...
		if (iterator > 0) {
			...
			final int diff = GLCellLayout.sLongAxisEndPadding - GLCellLayout.sLongAxisStartPadding;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0, j = 0; i < count; i++) {
			final GLCardLayout child = (GLCardLayout) getChildAt(i);
			if (child != null) {
				child.setId(i);
				final GLCardLayout replaceCard = (GLCardLayout) getChildAt(mRelpaceSrcIndex);
	...
	public void replaceCard(final int srcIndex, final int destIndex, List<Rect> newRects) {
		...
		for (int i = 0; i < size; i++) {
			...
			final GLCardLayout card = (GLCardLayout) getChildAt(abusoluteIndex);
		...
		//重置当前页索引
		if (GLSense.sCurScreenId == destIndex) {
			if (srcIndex > destIndex) {
				GLSense.sCurScreenId = destIndex + 1;
			} else {
				GLSense.sCurScreenId = destIndex - 1;
			}
		} else if (GLSense.sCurScreenId < maxIndex && GLSense.sCurScreenId > minIndex) {
			if (srcIndex > destIndex) {
				GLSense.sCurScreenId += 1;
			} else {
				GLSense.sCurScreenId -= 1;
			}
		} else if (GLSense.sCurScreenId == srcIndex) {
			GLSense.sCurScreenId = destIndex;
		}
...
//		handleEnterFinished();
	 ...
	 *            离开时间
	 */
	public void leaveCard(int leaveCardId, GLView view) {
		lock();
		mLeaveCardId = leaveCardId;
		setBackgroundColor(0);
		mLeavingView = view;
		//		if (mLeavingView instanceof GLCellLayout) {
		//			((GLCellLayout) mLeavingView).buildChildrenDrawingCache();
		//		}
	...
	private void handleLeaveFinish() {
		unlock();
		Assert.assertTrue(mLeavingView instanceof GLCellLayout);
		((GLCellLayout) mLeavingView).setChildrenDrawnWithCacheEnabled(false);
	...
	public void handleReplaceFinish() {
		...
		// TODO 更改所有相关的ID
		GLView srcView = getChildAt(mRelpaceSrcIndex);
		if (srcView == null) {
			return;
		}
		Message msg = new Message();
		msg.what = HANDLE_REPLACEFINISH;
	...
	private void completeReplaceFinish() {
		...
		if (mHasFinishRelpace) {
			getChildAt(mRelpaceDestIndex).setVisibility(GLView.VISIBLE);
	...
	private void drawReplacing(final GLCanvas canvas, long currentTime) {
		final int count = getChildCount();
	...
	private void drawRelaceBack(final GLCanvas canvas, long currentTime) {
		final int count = getChildCount();
		...
		if (currentTime >= mReplaceAnimationDuration) {
			GLCardLayout dragCard = (GLCardLayout) getChildAt(mDragCardIndex);
			if (dragCard != null) {
	...
	private void drawMoving(final GLCanvas canvas, long currentTime) {
		...
		for (int i = 0; i < count; i++) {
			...
			if ((mStatus == SENSE_MOVING)
				...
				if (getChildAt(i).getVisibility() == GLView.VISIBLE) {
					super.drawChild(canvas, child, getDrawingTime());
				}
			}
			canvas.restoreToCount(saveCount);
	...
	private void drawWaitForEnter(final GLCanvas canvas) {
		// do nothing
		return;
	}

	void drawView(GLView view, GLCanvas canvas) {
//		Bitmap bitmap = null;
//		try {
...
//		}
	...
	private void drawEntering(final GLCanvas canvas, long currentTime) {
...
//		canvas.drawColor(color);
		...
		final int beg = GLSense.sCurScreenId - GLSense.sCurScreenId % mRowsMulColumns;
	...
	private void drawNormal(GLCanvas canvas) {
		super.dispatchDraw(canvas);
	}

	private void drawLeaving(final GLCanvas canvas, final long currentTime) {
		final float t = Math.min(1, currentTime / (float) LEAVE_DURATION);
		int topD = StatusBarHandler.isHide() ? 0 : StatusBarHandler.getStatusbarHeight();
...
//		canvas.drawColor(color);
		...
		if (mLeavingView != null) {
			...
			canvas.translate(getScrollX(), 0 - topD);
	...
	private RectF getMoveDestination(GLView child) {
		final int index = indexOfChild(child);
		// 获取当前View的前一个View的位置
		final int fromIndex = index + 1;
		if (fromIndex < getChildCount()) {
			GLView preView = getChildAt(index + 1);
	...
	private RectF getRelpaceDestination(GLView child, final int index) {
		RectF rect = new RectF(child.getLeft(), child.getTop(), child.getRight(), child.getBottom());
		GLView destChild = null;
	...
	public void addView(GLView child) {
		if (!(child instanceof GLCardLayout)) {
	...
	public List<Rect> getCurScreenRects() {
		...
		for (int i = startIndex; i < endIndex; i++) {
			GLCardLayout card = (GLCardLayout) getChildAt(i);
			if (card.getVisibility() != GLView.GONE) {
	...
	public void hideAddCard() {
		...
		for (int i = 0; i < count; i++) {
			GLCardLayout card = (GLCardLayout) getChildAt(i);
			if (card.getType() == GLCardLayout.TYPE_ADD) {
				removeView(card);
				card.cleanup();
				sCardCount--;
				break;
	...
	public void hideCard(int cardId) {
		getChildAt(cardId).setVisibility(GLView.INVISIBLE);
	}

	public void showCard(int cardId) {
		if (getChildAt(cardId) != null) {
			getChildAt(cardId).setVisibility(GLView.VISIBLE);
		}
	}
	...
	public void showAddCard() {
		if (getChildCount() < MAX_CARD_NUMS) {
			...
			final int count = getChildCount();
			if (count <= 0
					|| ((GLCardLayout) getChildAt(count - 1)).getType() != GLCardLayout.TYPE_ADD) {
				addAddCardToScreen();
				sCardCount++;
			}
			// 更新指示器
	...
	public Rect enlargeCard(final int index) {
		...
		if (infactIndex < getChildCount()) {
			// 合法索引
			final GLCardLayout card = (GLCardLayout) getChildAt(infactIndex);
	...
	public void resumeCard(final int index) {
		final int infactIndex = getCurScreenIndex() * mRowsMulColumns + index;
		if (infactIndex < getChildCount()) {
			GLCardLayout cardLayout = (GLCardLayout) getChildAt(infactIndex);
			if (cardLayout != null) {
				cardLayout.resume();
			}
		}
	...
	public boolean isAddCard(final int index) {
		...
		if (infactIndex < getChildCount()) {
			GLCardLayout cardLayout = (GLCardLayout) getChildAt(infactIndex);
			if (cardLayout != null) {
				return cardLayout.getType() == GLCardLayout.TYPE_ADD;
			}
		}
		return false;
	}
	...
	public void onCardEvent(GLCardLayout layout, int event) {
		...
		switch (event) {
			...
			case ICardEvent.ADD_CLICK :
				...
				GuiThemeStatistics
						.goLauncherUserBehaviorStaticDataCache(IGoLauncherUserBehaviorStatic.SCREEN_ADD_20);
				break;
			...
			case ICardEvent.PREVIEW_LONG_CLICK :
				...
				//				layout.setVisibility(GLView.INVISIBLE);
	...
	void completeAddEmptyCard() {
		if (hasSpaceToAdd()) {
			if (mListener.preAddCard()) {
				//当加号屏为第九屏的时候不做动画
				if (addEmptyCardToScreen()) {
					TranslateAnimation animation = null;
					AnimationTask task = new AnimationTask(true, AnimationTask.PARALLEL);
					GLCardLayout addCard = (GLCardLayout) getChildAt(getChildCount() - 1);
					if (addCard.getId() != 2 && addCard.getId() != 5) {
						animation = new TranslateAnimation(-(sCardWidth + sSpaceX), 0, 0, 0);
					} else {
						animation = new TranslateAnimation(2 * (sCardWidth + sSpaceX), 0,
								-sCardHeight - sSpaceY, 0);
					}
					animation.setDuration(mReplaceAnimationDuration);
					animation.setFillEnabled(true);
					animation.setFillAfter(true);
					task.addAnimation(getChildAt(getChildCount() - 1), animation, null);
					AlphaAnimation alphaAnimation = new AlphaAnimation(0, 1);
					alphaAnimation.setDuration(150);
					GLCardLayout card = (GLCardLayout) getChildAt(getChildCount() - 2);
					if (card != null) {
						card.setHasPixelOverlayed(false);
						task.addAnimation(card, alphaAnimation, null);
	...
	void completeRemoveCard(GLCardLayout card) {
		// 作保护
		if (card == null) {
			return;
		}
		...
		if (getChildCount() > 1) {
			...
			if (card.isHome()) {
				// 第一屏为home屏
				final GLCardLayout newHomeCard = (GLCardLayout) getChildAt(0);
			...
			if (card.isCurrent()) {
				// 第一屏为当前屏
				((GLCardLayout) getChildAt(0)).setCurrent(true);
	...
	void completeSetHome(GLCardLayout card) {
		// 作保护
		if (card != null) {
			if (mHomeCard != null) {
	...
	boolean addEmptyCardToScreen() {
		boolean flag = true;
		GLCardLayout card = new GLCardLayout(getContext(), GLCardLayout.TYPE_PREVIEW, null, true,
				this);
		card.setEventListener(this);
		card.setPreviewController(mPreviewController);
		// 添加到倒数第二个，在+号之前
	...
	void addAddCardToScreen() {
		GLCardLayout cardLayout = new GLCardLayout(getContext(), GLCardLayout.TYPE_ADD, null,
				false, this);
	...
	public void enableChildrenDrawingCacheCurrently() {
		final int curIndex = GLSense.sCurScreenId;
		final int beg = curIndex - curIndex % mRowsMulColumns;
		final int end = Math.min(mRowsMulColumns + beg, getChildCount());
		for (int i = beg; i < end; i++) {
			final GLCardLayout layout = (GLCardLayout) getChildAt(i);
	...
	public void setEnableUpdate(boolean enable) {
		...
		if (enable) {
			...
			for (int i = 0; i < count; i++) {
				...
				//				layout.buildDrawingCache(canvas)
	...
	void showToast(int id) {
		try {
			ToastUtils.showToast(id, Toast.LENGTH_SHORT);
		} catch (OutOfMemoryError e) {
			OutOfMemoryHandler.handle();
		}
	}
...
//		setCaptionY();
	...
	void handleEnterFinished() {
		...
		mStatus = SENSE_NORMAL;
		//如果是从桌面拖入,则不显示提示层小白云
		if (!GLSense.sDragFromeScreen) {
			
			ScreenSettingInfo info = SettingProxy.getScreenSettingInfo();
			
			if (info == null || !info.mLockScreen) {
		...
		if (sNeedScaleWidget) {
			...
			if (getGLParent() instanceof GLSense) {
				((GLSense) getGLParent()).scaleWidget();
			}
			sNeedScaleWidget = false;
		}
	}
	...
	public void tutorialSetMainScreen(int index) {
		...
		GLCardLayout card = (GLCardLayout) getChildAt(index);
	...
	public void setEnoughSpaceCardBG(ArrayList<Integer> list) {
		...
		if (list.size() == 0) {
			card = (GLCardLayout) getChildAt(getChildCount() - 1);
			if (card.getType() == GLCardLayout.TYPE_ADD) {
				card = (GLCardLayout) getChildAt(getChildCount() - 1);
				...
				for (Integer index : noEnoughList) {
					card = (GLCardLayout) getChildAt(index);
					card.setNoRoom();
				}
			} else {
				ArrayList<Integer> noEnoughList = new ArrayList<Integer>();
	 ...
	 * @return
	 */
	public int getLastEffectiveScreenIndex() {
		final int count = getChildCount();
		if (isAddCard(count - 1)) {
			return count - 2;
		} else {
			return count - 1;
		}
	} // end getLastScreenIndex
