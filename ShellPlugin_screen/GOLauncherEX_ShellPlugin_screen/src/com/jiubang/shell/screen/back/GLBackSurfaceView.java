 ...
 * 
 */
public class GLBackSurfaceView extends SurfaceView implements SurfaceHolder.Callback {

	private GLBackSurfaceThread mThread;
	...
	public GLBackSurfaceView(Context context) {
		super(context);
		init();
	}

	public GLBackSurfaceView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public GLBackSurfaceView(Context context, AttributeSet attrs, int defStyle) {
	...
	private void startThread() {
		if (mThread == null) {
			mThread = new GLBackSurfaceThread(getHolder(), this);
	...
	public void onStateMethod(String state) {
		if (state.equals(GLBackWorkspace.STATE_ON_PAUSE) || state.equals(GLBackWorkspace.STATE_ON_STOP)
				|| state.equals(GLBackWorkspace.STATE_ON_DESTROY)) {
			stopThread();
		} else if (state.equals(GLBackWorkspace.STATE_ON_RESUME)) {
			startThread();
		}
	}
