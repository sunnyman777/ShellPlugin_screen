 ...
 *
 */
public class GLAddFolderTab extends GLGridTab implements BatchAnimationObserver, Callback {

	private GLView mFlyView;
	private ArrayList<AppItemInfo> mAppList;
	private ConcurrentHashMap<Intent, Boolean> mBooleanList;
			...
			switch (msg.what) {
				case ON_FLY_FINISH :
					...
					MsgMgrProxy.sendMessage(this, IDiyFrameIds.SCREEN,
							...
							ScreenEditConstants.ADD_FOLDER_ID, msg.obj);
	...
	public GLAddFolderTab(Context context, int tabId, int tabLevel) {
		super(context, tabId, tabLevel);
		mPreTabId = ScreenEditConstants.TAB_ID_MAIN;
		mNeedAsyncLoadImage = false;
		mBooleanList = new ConcurrentHashMap<Intent, Boolean>();
	}
	...
	public void onItemClick(GLAdapterView<?> adapter, GLView view, int position, long id) {
		...
		if (mBooleanList.get(info.mIntent)) {
			// 把图标从加入的文件夹中删除
			mBooleanList.put(info.mIntent, false);
			changeSelectedView(view, false);
			MsgMgrProxy.sendMessage(this, IDiyFrameIds.SCREEN,
					IScreenFrameMsgId.SCREEN_DEL_ITEM_FROM_FOLDER,
					ScreenEditConstants.ADD_FOLDER_ID, view);
			return;
		} else {

			// 把图标加入到新建的文件夹中 
			mBooleanList.put(info.mIntent, true);
			changeSelectedView(view, true);
			...
			if (checkScreenVacant(1, 1, centerXY, translate)) {
				// 进行添加的飞行动画
				flyToScreen(view, centerXY, translate);
			}
		}
	}
	 ...
	 * @param visible
	 */
	public void changeSelectedView(GLView view, boolean visible) {
		GLImageView image2 = (GLImageView) view.findViewById(R.id.screen_edit_item_state);
		if (visible) {
