...
public class MultiTouchDetector {
	 ...
	 * 移动超过10个PIX才认为是多指移动
	 */
	private static final float SWIPE_LIMITE_LIME = 100f;  // 差值
	...
	private OnMultiTouchGestureListener mListener;
	private PointInfo mPrevPoint = new PointInfo(); // 上次事件
	private PointInfo mCurrPoint = new PointInfo(); // 本次事件
	...
	private PointInfo mLastHistoryPoint;  // 上一次历史点击触摸点
	private ArrayList<PointInfo> mHistory = new ArrayList<PointInfo>(SAMPLE_SIZE);
	...
	private PointInfo mCurrDownPoint = new PointInfo();  // 当前按下
	private PointInfo mPrevUpPoint = new PointInfo();   // 上一次放手
	...
	public MultiTouchDetector(Context context, OnMultiTouchGestureListener listener) {
		mListener = listener;
		...
		// 当屏幕检测到有手指按下之后就触发到这个事件
			...
			case MotionEvent.ACTION_POINTER_DOWN : {
				...
				if (mLastHistoryPoint == null) {
					//addToHistory(mCurrPoint);// jamin mod
	...
	private void decodeHistoryState() {
		...
		if (size >= 2) {
			...
			// mTouchState);
			PointInfo startPoint = mHistory.get(0);  // 获取起点
	...
	private boolean notifyMultiTouchState(float dx, float dy) {
		if (!mPrevPoint.isMultiTouch() || !mCurrPoint.isMultiTouch()) {
			...
			// 历史数据已经判断出手势
			if (mStartDecodeMultiTouch) { //jamin mod 
		...
		if (mListener != null) {
			switch (mTouchState) {
				...
				case STATE_SCALE :   // 缩放状态
					...
					// degreeDelta);
					float xOffset2 = mCurrPoint.getX() - mPrevPoint.getX();
					float yOffset2 = mCurrPoint.getY() - mPrevPoint.getY();
					
					return mListener.onScale(mCurrPoint, scaleDelta, scaleDelta,
						scaleDelta, xOffset2, yOffset2, degreeDelta);
				...
				case STATE_SWIPE :   // 多指滑动状态
					...
					return mListener.onTwoFingerSwipe(mCurrPoint, xOffset, yOffset,
	...
	private int getMoveDirection(float dx, float dy) {
		if (Math.abs(dy) > Math.abs(dx)) {
			return dy > 0 ? OnMultiTouchGestureListener.DIRECTION_DOWN : OnMultiTouchGestureListener.DIRECTION_UP;
		} else {
			return dx > 0
					? OnMultiTouchGestureListener.DIRECTION_RIGHT
					: OnMultiTouchGestureListener.DIRECTION_LEFT;
		}
	}
	...
	public float getFingerDiff(PointInfo p) {
		return Math.max(MIN_MULTITOUCH_SEPARATION * 0.71f, p.getFingerDistance());
	}

	public float getFingerAngle(PointInfo p) {
		return PointInfo.radian2Degree(p.getFingerAngle());
	}
	...
	private boolean isSwipe(float p0dx, float p0dy, float p1dx, float p1dy) {
		...
		//add by Jamin 最小启动判定值
		if ((p0dx * p0dx + p0dy * p0dy) < SWIPE_LIMITE_LIME
				|| (p1dx * p1dx + p1dy * p1dy) < SWIPE_LIMITE_LIME) {
			return false;
		}
	 ...
	 * @param p
	 */
	private void addToHistory(PointInfo p) {
		PointInfo info = new PointInfo(p);
	 ...
	 * @return
	 */
	private boolean isConsideredAddToHistory(PointInfo current, PointInfo preview) {
	 ...
	 * @return
	 */
	private boolean isConsideredDoubleTap(PointInfo firstDown, PointInfo firstUp,
			PointInfo secondDown) {
