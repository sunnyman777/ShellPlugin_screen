...
//		boolean ret = super.dispatchTouchEvent(ev);
		...
		float tx = ev.getX();
		float ty = ev.getY();
		if (tx >= getScrollX() && tx <= getScrollX() + mWidth
			&& ty >= getScrollY() && ty <= getScrollY() + mHeight) {
			ret = true;
		} else {
			return false;
		}
