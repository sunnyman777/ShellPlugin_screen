 ...
 * @date  [2013-3-13]
 */
public class GLDockFolderGridVIew extends GLAppFolderBaseGridView<GLDockFolderIcon> implements BroadCasterObserver {
	...
	public GLDockFolderGridVIew(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
...
//	}
	...
	public void setGridRowsAndColumns() {
		mNumColumns = mDeskTopSetting.mColumn;
	}
	@Override
			...
			case GLView.GONE :
				...
				if (mPositionChange) {
					GLAppFolderController controller = GLAppFolderController.getInstance();
						controller.moveScreenFolderInnerItem(mFolderIcon.getUserFolderInfo());
	...
	public void onDropCompleted(DropTarget target, Object dragInfo, boolean success,
		...
		if (target != this) {
			MsgMgrProxy.sendMessage(this, IDiyFrameIds.DOCK,
					...
					mAdapter.getViewByItem(dragInfo));
		}
	}

	@Override
	public void onFolderNameChange(String name) {
		UserFolderInfo folderInfo = mFolderIcon.getFolderInfo().getScreenFoIderInfo();
		folderInfo.mTitle = name;
		folderInfo.setFeatureTitle(name);
		mFolderIcon.refreshIcon();
		mController.updateDockFolderItem(folderInfo.mInScreenId, mFolderIcon.getInfo());
	}
	@Override
	...
	public void showSortDialog() {
		...
		sortDialog.setOnItemClickListener(new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int item) {
				...
				List<?> folderContent = mFolderIcon.getFolderInfo().getScreenFoIderInfo()
						.getContents();
				switch (item) {
...
//		setData(mFolderIcon.getFolderInfo().getScreenFoIderInfo().getContents());
//		mFolderIcon.refreshIcon();
//	}

//	@Override
//	public int getContentSize() {
//		return mFolderIcon.getFolderInfo().getScreenFoIderInfo().getContents().size();
//	}
	@Override
