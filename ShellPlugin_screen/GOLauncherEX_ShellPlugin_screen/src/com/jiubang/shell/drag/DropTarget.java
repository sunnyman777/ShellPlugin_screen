 ...
 * 
 */
// TODO:setTopViewID,getTopViewID，等有更好的办法再去掉
//CHECKSTYLE OFF
public interface DropTarget {
	 ...
	 * 
	 */
	boolean onDrop(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView,
			Object dragInfo, DropAnimationInfo resetInfo);

	void onDragEnter(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView,
			Object dragInfo);

	void onDragOver(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView,
			Object dragInfo);

	void onDragExit(DragSource source, DropTarget nextTarget, int x, int y, int xOffset, int yOffset, DragView dragView,
			Object dragInfo);

	// 针对某些组件局部接受拖拽进出事件（例如垃圾箱组件只有左半部分响应删除）
	void onDragMove(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView,
			Object dragInfo);

	// 设置这个taget属于哪个顶层的view
	void setTopViewId(int id);
