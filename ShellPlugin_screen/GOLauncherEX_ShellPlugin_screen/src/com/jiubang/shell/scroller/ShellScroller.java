 ...
 * 
 */
public class ShellScroller extends ShellMScroller {

	ShellScrollerListener mListener;
	...
	ShellScrollerEffector mEffector;
	/**
	 * 是否是循环滚动
	 */
	 ...
	 * @param listener
	 */
	public ShellScroller(Context context, ShellScrollerListener listener) {
	 ...
	 */
	public ShellScroller(Context context, ShellScrollerListener listener, FastVelocityTracker tracker) {
		super(context);
			...
			case MotionEvent.ACTION_MOVE :
				...
				if (mState != ShellMScroller.ON_SCROLL) {
					if (Math.abs(mLastTouchP - mTouchDownP) >= mCurrentTouchSlop) {
						mTouchDownP = mLastTouchP;
						onScrollStart();
						mListener.onScrollStart(mScroll);
					}
				} else if (mState == ShellMScroller.ON_SCROLL) {
					onScroll(delta);
				}
				break;
	...
	protected void onComputeFlingOffset(float t) {
		...
		if (isFinished()) {
			mListener.onScrollFinish(mScroll);
		}
	}
	...
	private float getCurrVelocity() {
		long time = Timer.getTime(mContextHashCode);
	 ...
	 *            为null表示不使用效果
	 */
	public void setEffector(ShellScrollerEffector effector) {
		ShellScrollerEffector oldEffector = mEffector;
		mEffector = effector;
	...
	public final ShellScrollerEffector getEffector() {
		return mEffector;
	}
	 ...
	 * @return
	 */
	public void setCycleMode(ShellScrollerListener listener, boolean cycle, Context context) {
		if (listener == null) {
			return;
		}
		ShellScroller scroller = listener.getScroller();
		if (scroller != null && scroller.isCircular() == cycle) {
			return;
		}
		ShellScroller newScroller = cycle ? (new ShellCycloidScroller(context, listener)) : (new ShellScroller(
				context, listener));
	 ...
	 * @param newScroller
	 */
	private static void copyScrollerAttributes(ShellScroller scroller, ShellScroller newScroller) {
