 ...
 * {@link #setCycleMode(ShellScreenScrollerListener, boolean)}方法。
 * 
 * @author dengweiming
 * 
 */
public class ShellScreenScroller extends ShellMScroller {
	...
	protected ShellScreenScrollerListener mListener;
	...
	protected GLIWallpaperDrawer mWallpaperDrawer;
	 ...
	 * @param screenGroup
	 */
	public ShellScreenScroller(Context context, ShellScreenScrollerListener screenGroup) {
	...
	public ShellScreenScroller(Context context, ShellScreenScrollerListener screenGroup,
		...
		mInterpolatorBak = ShellMScroller.DEFAULT_INTERPOLATOR;
	...
	public void setInterpolator(Interpolator interpolator) {
		if (interpolator == null) {
			interpolator = ShellMScroller.DEFAULT_INTERPOLATOR;
		}
	 ...
	 *            为null表示不使用效果
	 */
	public void setEffector(ShellScreenScrollerEffector effector) {
		ShellScreenScrollerEffector oldEffector = mEffector;
		mEffector = effector;
	...
	protected boolean flingToScreen(int dstScreen, int duration) {
		...
		if (mScroll < 0 && dstScreen <= 0) {
			...
			interpolator = ShellMScroller.VISCOUS_FLUID_INTERPOLATOR;
	...
	protected boolean gotoScreen(int dstScreen, int duration, Interpolator interpolator) {
		mInterpolator = interpolator != null ? interpolator : ShellMScroller.DEFAULT_INTERPOLATOR;
		mDstScreen = checkScreen(dstScreen);
		final int delta = mDstScreen * mScreenSize - mScroll;
		if (delta == 0 && getCurrentDepth() == 0) {
			final int oldState = mState;
			if (mState != ShellMScroller.FINISHED) {
				mState = ShellMScroller.FINISHED;
				if (mEffector != null) {
			...
			return oldState != ShellMScroller.FINISHED;
		}
		if (mState == FINISHED) {
			if (mEffector != null) {
	...
	public void gotoScreen(int dstScreen, int duration, boolean noElastic) {
		...
		gotoScreen(dstScreen, duration < 0 ? mScrollingDuration : duration, noElastic
				? ShellMScroller.DEFAULT_INTERPOLATOR
				: mInterpolatorBak);
	}
	...
	public final ShellScreenScrollerEffector getEffector() {
		return mEffector;
	}
	...
	public boolean onTouchEvent(MotionEvent event, int action) {
		...
		switch (action) {
			case MotionEvent.ACTION_DOWN : {
				...
				if (mEnableInteruptFlip && mState == ShellMScroller.ON_FLING) {
					mState = ShellMScroller.TO_SCROLL;
					if (mEffector != null) {
			...
			case MotionEvent.ACTION_MOVE : {
				...
				if (!mEnableInteruptFlip && mState == ShellMScroller.ON_FLING) {
					...
					if (mState != ShellMScroller.ON_SCROLL) {
			...
			case MotionEvent.ACTION_CANCEL : {
				...
				if (mState == ShellMScroller.TO_SCROLL) {
	...
	public void setBackground(Drawable drawable, boolean isClearCurrent) {
		...
		Drawable oldDrawable = mBackgroundDrawable; // 保存旧的图
		...
		if (mBackgroundDrawable != null) {
			...
			// 横屏下是不需要x两个屏幕的宽度的
			final int factor = GoLauncherActivityProxy.isPortait() ? 2 : 1;
			mBackgroundWidth = DrawUtils.sWidthPixels * factor;
			mBackgroundHeight = DrawUtils.sHeightPixels;
		}
		
		if (mWallpaperDrawer != null) {
			int x = -getBackgroundOffsetX(mScroll);
			...
			mWallpaperDrawer.setBackground(mBackgroundDrawable, mBitmap);
			// 不能置为null，否则下次更换壁纸的时候无法释放内存
//			mBackgroundDrawable = null;
			mBitmap = null;
		}
		
		// TODO:由Workspace去释放
		if (isClearCurrent && oldDrawable != null && oldDrawable != drawable) {
			if (oldDrawable instanceof GLDrawable && !(oldDrawable instanceof BlurGLDrawable)) { // 模糊壁纸不在这里进行回收，避免一些错误
				((GLDrawable) oldDrawable).clear();
				oldDrawable = null;
			}
		}
	}
	...
	public int getBackgroundOffsetX(int scroll) {
		if (mBackgroundScrollEnabled) {
			float res = (mBackgroundWidth - DrawUtils.sWidthPixels) * (scroll - mMinScroll) * mScrollRatio;
			return (int) (res + 0.5f);  // CHECKSTYLE IGNORE THIS LINE
		} else {
			return (int) ((mBackgroundWidth - DrawUtils.sWidthPixels) / 2);
		}
	}
	...
	public boolean drawBackgroundOnScreen(GLCanvas canvas, int screen) {
		if (!mBgAlwaysDrawn && mWallpaperDrawer != null) {
			mWallpaperDrawer.setUpdateBackground(true);
			screen = checkScreen(screen);
			final int x = -getBackgroundOffsetX(screen * mScreenSize);
			int y = -mBackgroundOffsetY;
			if (mLayoutScale < 1.0f && mListener instanceof GLSuperWorkspace) {
				y -= ((GLSuperWorkspace) mListener).getTranslateY();
			}
	 ...
	 * @return
	 */
	public static void setCycleMode(ShellScreenScrollerListener listener, boolean cycle) {
		if (listener == null) {
			return;
		}
		ShellScreenScroller scroller = listener.getScreenScroller();
		if (scroller != null && scroller.isCircular() == cycle) {
			return;
		}
		ShellScreenScroller newScroller = cycle
				? (new ShellCycloidScreenScroller(null, listener))
				: (new ShellScreenScroller(null, listener));
	 ...
	 * @param newScroller
	 */
	private static void copyScrollerAttributes(ShellScreenScroller scroller,
			ShellScreenScroller newScroller) {
		...
		final Drawable wallpaper = scroller.mWallpaperDrawer == null
				? scroller.mBackgroundDrawable
				: scroller.mWallpaperDrawer.getBackgroundDrawable();
		newScroller.setBackground(wallpaper);
	...
	public boolean isOnFilng() {
		return mState == ShellMScroller.ON_FLING;
	}
