...
class CylinderEffector extends MGridScreenEffector {
	...
	public CylinderEffector() {
		setVerticalSlide(true); //圆柱特效默认打开垂直滑动效果
	}
	...
	public void onSizeChanged(int w, int h) {
		...
		mRatio = 1.0f / w;
		DisplayMetrics metrics = ApplicationProxy.getContext().getResources().getDisplayMetrics();
		if (metrics.density == 2.0) {
			mRadius = w / (float) Math.toRadians(HALF_ANGLE) * 1.45f; //CHECKSTYLE IGNORE
		} else {
			mRadius = w / (float) Math.toRadians(HALF_ANGLE) * 1.7f; //CHECKSTYLE IGNORE
		}
	}
